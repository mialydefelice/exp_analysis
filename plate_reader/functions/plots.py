...
def make_individual_plots_subplot(time, conditions, condition_names, df_merged, fig_save_path, y_max, xbounds, fig_name_header):
    ...
    for j in range(len(conditions)):
        ...
        for c in range(0, len(data_to_plot)):
            ...
            axs[j].fill_between(np.array(time), np.array(data)- np.array(error), 
                             np.array(data) + np.array(error), alpha=0.15, facecolor = color_palette[j])
            axs[j].set_ylim([0,y_max])
            axs[j].set_xlim(xbounds)
            axs[j].set_xlabel("Time (min)")
            axs[j].set_ylabel("OD_600")
            axs[j].legend()
    sns.despine()
    fig.savefig(os.path.join(fig_save_path, fig_name_header + '.pdf'), bbox_inches='tight')
    plt.show()
    plt.clf()
    return